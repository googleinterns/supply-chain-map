...
import { FormQueryResult } from 'src/app/services/home-helper/home-helper.service';

/**
 * 
 */
interface RouteLayerMarker {
  lat: number;
  long: number;
  iconUrl: string;
  type: string[];
...
export class RouteLayerComponent implements OnInit {
  ...
  createRoutes(formQueryResult: FormQueryResult) {
    this.createMarkerPoints(formQueryResult);
    this.createLines(formQueryResult);
  }

  private createLines(formQueryResult: FormQueryResult) {
  ...
  private createMarkerPoints(formQueryResult: FormQueryResult) {
    ...
    for (const latLongId of markerMap.keys()) {
      ...
      if (marker.type.length === 1) {
        switch (marker.type[0]) {
          case 'MFG': {
            marker.iconUrl = RouteLayerComponent.ICON_MAP.MFG;
            break;
          }
          case 'CM': {
            marker.iconUrl = RouteLayerComponent.ICON_MAP.CM;
            break;
          }
          case 'GDC': {
            marker.iconUrl = RouteLayerComponent.ICON_MAP.GDC;
            break;
          }
        }
        ...
        marker.iconUrl = RouteLayerComponent.ICON_MAP.MFG_CM;
      }
